    ...
    <div
        ...
        <div class="mb-4 flex flex-col gap-2 px-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end mt-2">
                <a wire:navigate href="{{ route('monthly-targets.create') }}"
                    class="bg-brand-600 hover:bg-brand-700 text-white font-medium px-6 py-2 rounded-md text-sm shadow-md transition duration-300 ease-in-out transform hover:scale-105"
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <div class="sm:w-48">
                    <select wire:model.live="statusFilter"
                        class="w-full rounded-lg border border-gray-300 bg-white px-3 pr-8 py-1 text-sm text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
        ...
        <div class="max-w-full overflow-x-auto">
            <div class="min-w-[1102px]">
                <!-- Table Header -->
                <div
                    class="grid grid-cols-12 border border-gray-200 dark:border-gray-600 text-theme-xs font-medium text-gray-700 dark:text-gray-200">
                    <div class="col-span-1 flex items-left px-4 py-3">ID</div>
                    <div class="col-span-2 flex items-left px-4 py-3">Month</div>
                    <div class="col-span-1 flex items-left px-4 py-3">Year</div>
                    <div class="col-span-3 flex items-left px-4 py-3">Target Amount</div>
                    <div class="col-span-3 flex items-left px-4 py-3">Description</div>
                    <div class="col-span-1 flex items-left px-4 py-3">Status</div>
                    <div class="col-span-1 flex items-left px-4 py-3">Action</div>
                </div>

                <!-- Table Body -->
                @forelse ($monthlyTargets as $monthlyTarget)
                    <div
                        class="grid grid-cols-12 border border-gray-100 dark:border-gray-600 text-theme-xs text-gray-700 dark:text-gray-200 py-2 dark:bg-gray-700">
                        <div class="col-span-1 px-4 flex items-left">{{ $monthlyTarget->id }}</div>
                        <div class="col-span-2 px-4 flex items-left">{{ $monthlyTarget->month }}</div>
                        <div class="col-span-1 px-4 flex items-left">{{ $monthlyTarget->year }}</div>
                        <div class="col-span-3 px-4 flex items-left">{{ $monthlyTarget->formatted_target_amount }}</div>
                        <div class="col-span-3 px-4 flex items-left">{{ $monthlyTarget->description ?: 'No description' }}</div>
                        <div class="col-span-1 flex items-left px-4">
                            @if ($monthlyTarget->status === 'active')
                                <span
                        ...
                        <div class="col-span-1 flex justify-center items-center px-4">
                            <div class="flex items-center gap-2">
                                <!-- Edit Button -->
                                <a href="{{ route('monthly-targets.edit', $monthlyTarget->id) }}" wire:navigate
                                    class="text-gray-500 hover:text-brand-700 dark:text-gray-400 dark:hover:text-white/90 text-sm font-medium"
                                    title="Edit">
                                ...
                                @if (auth()->user()->mode === 'admin')
                                    <button wire:click="confirmDelete({{ $monthlyTarget->id }})"
                ...
                @empty
                    <div class="py-6 px-4 text-gray-500 text-sm">No monthly targets found.</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="flex justify-between items-center border-t px-6 py-4 dark:border-gray-800">
            <div class="text-sm text-gray-600 dark:text-gray-400">
                Showing {{ $monthlyTargets->firstItem() }} to {{ $monthlyTargets->lastItem() }} of {{ $monthlyTargets->total() }} entries
            </div>

            <div class="pagination flex justify-between">
                {{ $monthlyTargets->links('vendor.pagination.custom-tailwind') }}
            </div>
        </div>
        ...
        @if($confirmingDelete)
            <div class="fixed inset-0 flex items-center justify-center p-5 overflow-y-auto z-50">
                <div class="fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[30px]"></div>
                    ...
                    <div class="relative w-full max-w-[507px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                        <div class="text-center">
                            ...
                            <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                                Are you sure you want to delete this monthly target? This action cannot be undone.
                            </p>
                            ...
                            <div class="flex items-center justify-center w-full gap-3 mt-8">
                                ...
                                <button type="button" wire:click="deleteMonthlyTarget"
